...
PROJECT_NAME=imageeditor-test #可执行程序的文件名
PROJECT_REALNAME=imageeditor  #项目名称
mkdir -p html
...
mkdir -p tests/test
     ...
     ./tests/${PROJECT_NAME} --gtest_output=xml:./report/report_imageeditor.xml
     lcov --directory . --capture --output-file ./html/${PROJECT_REALNAME}_Coverage.info

     echo " =================== do filter begin ==================== "
     lcov --remove ./html/${PROJECT_REALNAME}_Coverage.info 'CMakeFiles/${PROJECT_NAME}.dir/imageeditor-test_autogen/*/*' '${PROJECT_NAME}_autogen/*/*' 'googletest/*/*' '*/usr/include/*' '*/tests/*' '/usr/local/*' -o ./html/${PROJECT_REALNAME}_Coverage_fileter.info
     echo " =================== do filter end ==================== "
